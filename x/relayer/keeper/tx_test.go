...
func (suite *KeeperTestSuite) TestMsgServerNewVoter() {
	...
	txKeyHash2 := goatcrypto.SHA256Sum(suite.Voters[2].VoteKey)
	txKeyHash3 := goatcrypto.SHA256Sum(suite.Voters[3].VoteKey)
	err = suite.Keeper.ProcessRelayerRequest(suite.Context, goattypes.RelayerRequests{
		Adds: []*goattypes.AddVoterRequest{
			{Voter: common.Address(suite.Voters[2].Address), Pubkey: common.Hash(txKeyHash2)},
			{Voter: common.Address(suite.Voters[3].Address), Pubkey: common.Hash(txKeyHash3)},
		},
	})
	suite.Require().NoError(err)

	{
		...
		sigdoc := slices.Concat(
			...
			types.NewOnBoardingVoterRequest(uint64(sdkctx.BlockHeight()), account.GetAddress(), txKeyHash2).SignDoc(),
		)
	...
	{
		prvkey := &secp256k1.PrivKey{Key: suite.VoterKeys[3].TxKey}
		account, err := authtypes.NewBaseAccountWithPubKey(prvkey.PubKey())
		suite.Require().NoError(err)
		...
		sigdoc := slices.Concat(
			...
			[]byte("Relayer/NewVoter"),
			[]byte(suite.VoterKeys[0].Address),
			types.NewOnBoardingVoterRequest(uint64(sdkctx.BlockHeight()), account.GetAddress(), txKeyHash3).SignDoc(),
		)
		txKeyProof, err := prvkey.Sign(sigdoc)
		suite.Require().NoError(err)

		voteKeyProof := goatcrypto.Sign(new(goatcrypto.PrivateKey).Deserialize(suite.VoterKeys[3].VoteKey), goatcrypto.SHA256Sum(sigdoc))
		...
		_, err = server.NewVoter(suite.Context, &types.MsgNewVoterRequest{
			Proposer:         suite.VoterKeys[0].Address,
			VoterBlsKey:      suite.Voters[3].VoteKey,
			VoterTxKey:       prvkey.PubKey().Bytes(),
			VoterTxKeyProof:  txKeyProof,
			VoterBlsKeyProof: voteKeyProof,
		})
		...
		voter, err := suite.Keeper.Voters.Get(suite.Context, suite.VoterKeys[3].Address)
		suite.Require().NoError(err)
		suite.Require().Equal(voter, types.Voter{
			Address: suite.Voters[3].Address,
			VoteKey: suite.Voters[3].VoteKey,
			Status:  types.VOTER_STATUS_OFF_BOARDING,
			Height:  uint64(sdkctx.BlockHeight()),
		})
		...
		suite.Require().Equal(queue, types.VoterQueue{
			OnBoarding:  []string{suite.VoterKeys[2].Address},
			OffBoarding: []string{suite.VoterKeys[3].Address},
		})
	}
}
